...
export default {
	...
	methods: {
		resetForms() {
			...
			_this.v$.$reset()
			_this.restPassword = 1
			Object.assign(_this.formData, _this.initialFormData())
		},
		signIn () {
			const _this = this
			_this.v$.user_email.$touch()
			_this.v$.user_password.$touch()
			if (
				!_this.v$.user_email.$invalid
				&&
				!_this.v$.user_password.$invalid
			) {
				$fetch(`/api/user/signin`, {
					...
					if (data.result === null) {
						_this.signInError = '' + _this.t('wrong_credentials')
					} else {
						_this.signInError = '' + _this.t('there_was_an_error')
					}
				})
			}
		},
		register () {
			const _this = this
			_this.v$.register_first_name.$touch()
			_this.v$.register_last_name.$touch()
			_this.v$.register_email.$touch()
			_this.v$.register_password.$touch()
			_this.v$.register_confirm_password.$touch()
			_this.v$.register_terms_conditions.$touch()
			if (
				!_this.v$.register_first_name.$invalid
				&&
				!_this.v$.register_last_name.$invalid
				&&
				!_this.v$.register_email.$invalid
				&&
				!_this.v$.register_password.$invalid
				&&
				!_this.v$.register_terms_conditions.$invalid
				&&
				!_this.v$.register_terms_conditions.$invalid
			) {
				$fetch(`/api/user/create`, {
					...
					if (data.result === null) {
						_this.registrationError = '' + _this.t('email_taken')
					} else {
						_this.registrationError = '' + _this.t('there_was_an_error')
					}
				})
			}
		}
		...
		<UIModalWrapper @close="$emit('close')">
			...
			<div
				...
				<a
					href="#"
					class="font-avenir-next-book text-sm text-gray underline"
					@click="resetForms()">{{ t('forgot_password') }} {{ t('click_here') }}.</a>
				<div class="w-full mt-3">
					<a
						role="button"
						class="inline-block bg-green-7 hover:bg-green-5 font-avenir-next-heavy text-sm md:text-base text-white uppercase px-8 pt-3 pb-2 transition-colors rounded-md"
